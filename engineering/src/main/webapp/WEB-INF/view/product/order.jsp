...
<c:forEach items="${list }" var="productCartDTO">
	<tr><td><img src="../product/upload/${productCartDTO.productDTO.prodImage.split(',')[0] }" /></td>
		<td>${productCartDTO.productDTO.prodName }<br />
			단가 : <fmt:formatNumber value="${productCartDTO.productDTO.prodPrice }" pattern="#,###,###,###"/>원 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			${productCartDTO.cartDTO.cartQty }개
		</td>
		<td align="right"><fmt:formatNumber value="${productCartDTO.cartDTO.cartPrice }" pattern="#,###,###,###"/>원</td></tr>
...
<c:set var="proNums" value="${proNums += productCartDTO.cartDTO.prodNo += ','}" />
	...
	<tr><td colspan="3" align="right">최종 결제 금액<br /><fmt:formatNumber value="${totalPrice}" pattern="#,###,###,###"/>원</td></tr>
</table>
